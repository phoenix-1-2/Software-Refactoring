...
public class OMOPSO extends Algorithm
{
	...
	private void computeSpeed() throws JMException
	{
		...
		for (int i = 0; i < particlesSize_; i++)
		{
			...
			for (int var = 0; var < particle.length; var++)
			{
				...
				speed_[i][var] = W * speed_[i][var] + C1 * r1 * (bestParticle[var].getValue() - particle[var].getValue()) + C2 * r2 * (bestGlobal[var].getValue() - particle[var].getValue());
			}
	...
	private void mopsoMutation(int actualIteration, int totalIterations) throws JMException
	{
		...
		for (int i = 0; i < particles_.size(); i++)
			if (i % 3 == 0)
			{ // particles_ mutated with a non-uniform mutation
				nonUniformMutation_.execute(particles_.get(i));
			}
			else if (i % 3 == 1)
			{ // particles_ mutated with a uniform mutation operator
