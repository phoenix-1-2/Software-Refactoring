...
public abstract class Settings
{
	...
	public final Algorithm configure(Properties settings) throws JMException, IllegalArgumentException, IllegalAccessException, ClassNotFoundException
	{
		...
		if (settings != null)
		{
			...
			for (int i = 0; i < fields.length; i++)
			{
				...
				{ // it is a configuration field
					...
					else if (fields[i].getType().equals(double.class) || fields[i].getType().equals(Double.class))
					{
						...
						if (fields[i].getName().equals("mutationProbability_") && value == 0)
						{
							if ((problem_.getSolutionType().getClass() == Class.forName("jmetal.base.solutionType.RealSolutionType")) || (problem_.getSolutionType().getClass() == Class.forName("jmetal.base.solutionType.ArrayRealSolutionType")))
							{
								value = 1.0 / problem_.getNumberOfVariables();
							}
							else if (problem_.getSolutionType().getClass() == Class.forName("jmetal.base.solutionType.BinarySolutionType") || problem_.getSolutionType().getClass() == Class.forName("jmetal.base.solutionType.BinaryRealSolutionType"))
							{
			...
			for (int i = 0; i < fields.length; i++)
			{
				if (fields[i].getType().equals(jmetal.base.operator.crossover.Crossover.class) || fields[i].getType().equals(jmetal.base.operator.mutation.Mutation.class))
				{
						...
						if (i != j)
						{
							if (fields[j].getName().startsWith(aux))
							{
								...
								if ((fields[j].get(this) != null))
								{
									...
									else if (fields[j].getType().equals(double.class) || fields[j].getType().equals(Double.class))
									{
