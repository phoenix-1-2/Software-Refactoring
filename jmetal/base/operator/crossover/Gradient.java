...
public class Gradient extends Crossover
{
	...
	public Solution[] doCrossover(Solution parent1, Solution parent2) throws JMException
	{
		...
		for (int j = 0; j < offSpring.length; j++)
		{
			for (int i = 0; i < parent1.numberOfVariables(); i++)
			{
				double newValue = offSpring[j].getDecisionVariables()[i].getValue() - direccion[i];
	...
	public Object execute(Object object) throws JMException
	{
		...
		if ((parents[0].getType().getClass() != REAL_SOLUTION) || (parents[1].getType().getClass() != REAL_SOLUTION))
		{

			Configuration.logger_.severe("SBXCrossover.execute: the solutions " + "are not of the right type. The type should be 'Real', but " + parents[0].getType() + " and " + parents[1].getType() + " are obtained");
