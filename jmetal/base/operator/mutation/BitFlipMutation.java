...
public class BitFlipMutation extends Mutation
{
	...
	public void doMutation(double probability, Solution solution) throws JMException
	{
		try
		{
			if ((solution.getType().getClass() == BINARY_SOLUTION) || (solution.getType().getClass() == BINARY_REAL_SOLUTION))
			{
			...
			{ // Integer representation
				for (int i = 0; i < solution.getDecisionVariables().length; i++)
					if (PseudoRandom.randDouble() < probability)
					{
						int value = (int) (PseudoRandom.randInt((int) solution.getDecisionVariables()[i].getUpperBound(), (int) solution.getDecisionVariables()[i].getLowerBound()));
	...
	public Object execute(Object object) throws JMException
	{
		...
		if ((solution.getType().getClass() != BINARY_SOLUTION) && (solution.getType().getClass() != BINARY_REAL_SOLUTION) && (solution.getType().getClass() != INT_SOLUTION))
		{
		...
		if (probability == null)
		{
			Configuration.logger_.severe("BitFlipMutation.execute: probability not " + "specified");
			Class<?> cls = java.lang.String.class;
			String name = cls.getName();
			throw new JMException("Exception in " + name + ".execute()");
		}
